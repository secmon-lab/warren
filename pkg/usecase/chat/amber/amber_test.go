...
func TestAmberChat_ToolFiltering(t *testing.T) {
	...
	filtered := amber.FilterToolSets(ctx, allTools, []string{"tool_alpha", "tool_gamma"})
...
func TestAmberChat_ToolFilteringEmptyAllowList(t *testing.T) {
	...
	filtered := amber.FilterToolSets(ctx, allTools, []string{})
	gt.True(t, filtered == nil)
}

func TestAmberChat_SubAgentFiltering(t *testing.T) {
	...
	filtered := amber.FilterSubAgents(all, []string{"falcon", "slack"})
	gt.A(t, filtered).Length(2)
...
func TestAmberChat_BudgetMiddlewareNotAccumulated(t *testing.T) {
	...
	chatUC := amber.New(repo, mockLLM, newMockPolicyClient(t),
		...
		amber.WithTools([]gollem.ToolSet{testToolSet}),
	)
