...
func TestClusteringCachePerformance(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("cache effectiveness", func(t *testing.T) {
		...
		for i := 0; i < 20; i++ {
			testAlert := &alert.Alert{
				ID:       types.NewAlertID(),
				TicketID: types.EmptyTicketID,
				Schema:   "cache.test.v1",
				Data:     map[string]interface{}{"index": i},
				Metadata: alert.Metadata{
					Title:       fmt.Sprintf("Cache Test Alert %d", i),
					Description: "Alert for cache testing",
					Attributes:  []alert.Attribute{},
				},
				CreatedAt: time.Now().Add(-time.Duration(i) * time.Minute),
				Embedding: firestore.Vector32{
		...
		start1 := time.Now()
		result1, err := uc.ClusteringUC.GetAlertClusters(ctx, params)
		duration1 := time.Since(start1)
		gt.NoError(t, err)

		// Second call - should use cache
		start2 := time.Now()
		result2, err := uc.ClusteringUC.GetAlertClusters(ctx, params)
		...
		if len(result1.Clusters) != len(result2.Clusters) {
			t.Errorf("Cluster counts differ: first=%d, second=%d", len(result1.Clusters), len(result2.Clusters))
		}

		speedup := float64(duration1) / float64(duration2)
		t.Logf("First call: %v, Second call (cached): %v, Speedup: %.2fx",
