...
- `title`: Title of the alert set (around 100 characters max). This should be human readable and concise, clearly indicating what actor/subject performed what action against which resource/target and what potential impact occurred. The title should follow the pattern "Actor/Subject → Action → Resource/Target → Impact" where possible. In particular, generate a title that makes the type of alert and threat immediately apparent.
