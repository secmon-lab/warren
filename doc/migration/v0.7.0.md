...
### HTTP API Authorization: `auth` → `auth.http`
...
policies/
└── auth/
    ├── http.rego    # HTTP API authorization
    └── agent.rego   # Agent execution authorization
```

### Authorization Checklist

- [ ] Update HTTP API authorization policy package: `auth` → `auth.http`
...
- **Documentation**: See [Policy Guide](../operation/policy.md) for complete policy guide
